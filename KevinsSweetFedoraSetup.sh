#!/usr/bin/env bash
# ╔══════════════════════════════════════════════════════════════════════════════╗
# ║         KEVIN'S SWEET FEDORA SETUP — ELITE DEVELOPER ENVIRONMENT           ║
# ║         Fedora-Optimized · NVIDIA RTX 5060 · Catppuccin Mocha              ║
# ║         .NET / Next.js / Python · Kitty · ZSH · Hyprland + GNOME           ║
# ║         Version: 1.0.0                                                      ║
# ╚══════════════════════════════════════════════════════════════════════════════╝
# Usage:
#   chmod +x KevinsSweetFedoraSetup.sh
#   ./KevinsSweetFedoraSetup.sh             # full install
#   ./KevinsSweetFedoraSetup.sh --dry-run   # preview without changes
#   ./KevinsSweetFedoraSetup.sh --validate  # check install status only
#
# Run as your REGULAR USER (NOT root). sudo will be invoked where needed.
# Fedora uses dnf as the package manager; RPM Fusion (free + nonfree) is
# enabled automatically for packages not in the main repos (NVIDIA, etc.).
# Flatpak (Flathub) is used for apps not available in RPM form.
# ──────────────────────────────────────────────────────────────────────────────
...
LOGFILE="${HOME}/.local/share/kevins-fedora-setup.log"
...
dnf_install() {
    if [[ "$DRY_RUN" == "true" ]]; then info "[DRY-RUN] dnf install -y $*"; return 0; fi
    sudo dnf install -y "$@" 2>&1 | tee -a "$LOGFILE"
}

flatpak_install() {
    # Installs from Flathub; used for apps not packaged as RPMs
    if [[ "$DRY_RUN" == "true" ]]; then info "[DRY-RUN] flatpak install -y flathub $*"; return 0; fi
    flatpak install -y flathub "$@" 2>&1 | tee -a "$LOGFILE"
}
...
preflight() {
    ...
    if grep -qi 'fedora' /etc/os-release 2>/dev/null; then
        ok "Fedora detected: $(grep PRETTY_NAME /etc/os-release | cut -d= -f2 | tr -d '"')"
    else
        fail "This script requires Fedora Linux."
    fi

    step "Checking kernel..."
    KERNEL=$(uname -r)
    info "Kernel: $KERNEL"
    # Fedora ships with a vanilla kernel; note if a custom kernel is present
    if echo "$KERNEL" | grep -qiE 'zen|hardened|rt|liquorix'; then
        ok "Custom kernel detected: $KERNEL"
    else
        ok "Fedora kernel: $KERNEL"
    fi
    ...
    step "Enabling RPM Fusion repositories (free + nonfree)..."
    # RPM Fusion provides NVIDIA drivers, codecs, and other packages not in
    # Fedora's main repos. Required before any NVIDIA or media package installs.
    if [[ "$DRY_RUN" != "true" ]]; then
        ...
        info "[DRY-RUN] Enable RPM Fusion free + nonfree"
    fi

    step "Enabling Flathub (Flatpak)..."
    # Flathub provides apps not available as RPMs (DBGate, JetBrains Toolbox, etc.)
    if [[ "$DRY_RUN" != "true" ]]; then
        sudo dnf install -y flatpak 2>&1 | tee -a "$LOGFILE" || true
        flatpak remote-add --if-not-exists flathub https://dl.flathub.org/repo/flathub.flatpakrepo \
            2>&1 | tee -a "$LOGFILE" || true
        ok "Flathub enabled"
    else
        info "[DRY-RUN] Enable Flathub remote"
    fi
...
install_base_packages() {
    ...
    if [[ "$DRY_RUN" != "true" ]]; then
        ...
        info "[DRY-RUN] sudo dnf upgrade -y"
    fi
    ok "System updated"
    ...
    # lsd is not in Fedora repos; install via cargo or the copr
    step "Installing lsd (LSDeluxe) via dnf copr..."
    if [[ "$DRY_RUN" != "true" ]]; then
        sudo dnf copr enable -y atim/lsd 2>&1 | tee -a "$LOGFILE" || true
        sudo dnf install -y lsd 2>&1 | tee -a "$LOGFILE" || warn "lsd unavailable — install manually: cargo install lsd"
    else
        info "[DRY-RUN] Enable copr:atim/lsd and dnf install lsd"
    fi
...
setup_nvidia() {
    ...
    # On Fedora, NVIDIA drivers come from RPM Fusion (enabled in preflight).
    # RTX 5060 (Blackwell / GB206) requires the open-source kernel module
    # variant — available as akmod-nvidia (auto-builds against any kernel) or
    # the explicit kmod-nvidia-open packages. akmod-nvidia is the recommended
    # Fedora approach as it handles kernel updates automatically.
    #
    # Strategy:
    #   1. Remove any conflicting nouveau / old NVIDIA packages
    #   2. Install akmod-nvidia + xorg-x11-drv-nvidia (open kernel module)
    #   3. Install Wayland/Vulkan support packages
    #   4. Enable DRM modesetting via kernel cmdline (grubby)
    #   5. Rebuild initramfs with dracut

    step "Removing any conflicting NVIDIA / nouveau packages..."
    if [[ "$DRY_RUN" != "true" ]]; then
        sudo dnf remove -y xorg-x11-drv-nouveau 2>/dev/null || true
    fi

    step "Installing akmod-nvidia (RTX 5060 / Blackwell open module)..."
    dnf_install \
        akmod-nvidia \
        xorg-x11-drv-nvidia \
    ...
    if [[ "$DRY_RUN" != "true" ]]; then
        ...
        # Fedora uses grubby to modify kernel cmdline persistently
        sudo grubby --update-kernel=ALL --args="nvidia-drm.modeset=1" \
            2>&1 | tee -a "$LOGFILE" || true
        # Fedora uses dracut for initramfs
        sudo dracut --force 2>&1 | tee -a "$LOGFILE"
    else
        info "[DRY-RUN] write $modprobe_conf, grubby --args nvidia-drm.modeset=1, dracut --force"
    fi
...
setup_fonts() {
    ...
    # Fedora packages Nerd Fonts under jetbrains-mono-fonts; the full Nerd Font
    # variant is available via the jetbrains-mono-fonts RPM or manual install.
    dnf_install jetbrains-mono-fonts-all || true
    # Also install via the upstream Nerd Fonts archive for complete glyph coverage
    if [[ "$DRY_RUN" != "true" ]]; then
        ...
        if [[ ! -d "$FONT_DIR" ]]; then
            ...
            curl -sSL \
                ...
                && tar -xJf "$tmpdir/JetBrainsMono.tar.xz" -C "$FONT_DIR" \
                || warn "Nerd Font download failed — RPM fallback in use"
            rm -rf "$tmpdir"
        else
            ok "JetBrainsMono Nerd Font already present in $FONT_DIR"
        fi
    fi
...
setup_gnome_theme() {
    ...
    step "Installing GNOME Extension Manager via Flatpak..."
    flatpak_install com.mattjakeman.ExtensionManager || warn "Extension Manager unavailable — non-fatal"
    ...
    # Bibata is available on Flathub or via direct RPM; try dnf first
    dnf_install bibata-cursor-themes 2>/dev/null \
        || flatpak_install org.bibata.Cursor 2>/dev/null \
        || warn "Bibata cursor not found — install manually later"
...
install_dotnet() {
    ...
    # Fedora ships .NET via the Microsoft package feed (packages.microsoft.com)
    # or the Fedora-native dotnet packages. The Fedora repo provides .NET 8/9/10.
    step "Adding Microsoft .NET repo (packages.microsoft.com)..."
    if [[ "$DRY_RUN" != "true" ]]; then
        local fedora_ver; fedora_ver=$(rpm -E %fedora)
        sudo dnf install -y \
            "https://packages.microsoft.com/config/fedora/${fedora_ver}/packages-microsoft-prod.rpm" \
            2>&1 | tee -a "$LOGFILE" || true
    else
        info "[DRY-RUN] Install packages-microsoft-prod.rpm"
    fi
...
install_nodejs() {
    ...
    # Fedora ships Node.js LTS via the nodejs module stream
...
# ╔══════════════════════════════════════════════════════╗
# ║  Kevin's Kitty — Catppuccin Mocha · Fedora           ║
# ╚══════════════════════════════════════════════════════╝
...
configure_starship() {
    ...
    # starship was installed via dnf in Section 1
...
# ╔══════════════════════════════════════════════════════╗
# ║  Starship — Catppuccin Mocha Powerline · Fedora      ║
# ╚══════════════════════════════════════════════════════╝
...
# ╔══════════════════════════════════════════════════════════════════════════════╗
# ║  .zshrc — Kevin's Fedora Dev Shell · Catppuccin Mocha · Chuck Norris        ║
# ╚══════════════════════════════════════════════════════════════════════════════╝
...
alias dnforphans='sudo dnf autoremove -y'
# Aliases with familiar paru/pacman-style names for muscle memory
alias pacs='sudo dnf install -y'
alias pacr='sudo dnf remove -y'
alias pacu='sudo dnf upgrade -y'
alias pacss='dnf search'
alias pacsi='dnf info'
alias pacq='rpm -qa'
alias pacql='rpm -ql'
alias pacqo='rpm -qf'
alias pacclean='sudo dnf clean all'
alias pacorphans='sudo dnf autoremove -y'
alias pars='sudo dnf install -y'
alias parss='dnf search'
alias parr='sudo dnf remove -y'
alias yays='sudo dnf install -y'
alias yayu='sudo dnf upgrade -y'
alias yayss='dnf search'
alias mirror='sudo dnf --refresh upgrade -y'
...
" ╔══════════════════════════════════════════════════════╗
" ║  Kevin's Vim IDE — Catppuccin Mocha · Fedora         ║
" ╚══════════════════════════════════════════════════════╝
...
autocmd FileType python     nnoremap <buffer> <F9> :!python3 %<CR>
...
configure_fastfetch() {
    ...
    step "Writing fastfetch config (Fedora + Catppuccin Mocha)..."
    cat > "$HOME/.config/fastfetch/config.jsonc" << 'FFEOF'
{
    ...
    "display": {
        "separator": "  ",
        "color": { "keys": "blue", "title": "blue", "output": "cyan" }
    },
    "modules": [
...
FFEOF
    ok "fastfetch config written (Fedora logo)"
}
...
configure_hyprland() {
    ...
    # Hyprland is available in Fedora repos from F39+ via the official packages
    dnf_install \
        hyprland \
    ...
    # Fedora extras for Hyprland available via Flatpak or COPR
    step "Installing Hyprland extras via Flatpak / COPR..."
    if [[ "$DRY_RUN" != "true" ]]; then
        # hyprpaper and wlogout may be in fedora repos or available via copr
        sudo dnf install -y hyprpaper wlogout swaylock-effects 2>/dev/null \
            || warn "Some Hyprland extras unavailable in main repos — non-fatal"
    else
        info "[DRY-RUN] dnf install hyprpaper wlogout swaylock-effects"
    fi
...
# ╔══════════════════════════════════════════════════════╗
# ║  Kevin's Hyprland — Catppuccin Mocha · Fedora        ║
# ╚══════════════════════════════════════════════════════╝
...
install_extra_apps() {
    ...
    # DBGate is available as a Flatpak on Flathub
    flatpak_install io.github.dbgate.dbgate || warn "DBGate unavailable — install manually later"

    step "Installing JetBrains Toolbox..."
    # JetBrains Toolbox is available as a Flatpak on Flathub
    flatpak_install com.jetbrains.Toolbox || warn "JetBrains Toolbox unavailable — install manually later"
...
validate_installation() {
    ...
    if rpm -q akmod-nvidia &>/dev/null; then
        ...
        warn "akmod-nvidia not found (reboot may be needed)"
        ((warnings++)) || true
    fi

    info "── Fedora Kernel ──"
    local kver; kver=$(uname -r)
    if echo "$kver" | grep -qiE 'zen|hardened|rt'; then
        ok "Custom kernel: $kver"
    else
        ok "Fedora kernel: $kver"
    fi
...
# ╚══════════════════════════════════════════════════════════════════════════════╝
main() {
    banner "KEVIN'S SWEET FEDORA SETUP v${SCRIPT_VERSION}"
    ...
    banner "ALL DONE ON FEDORA, BOSS 🎉"

    echo -e "${GREEN}${BOLD}"
    echo "  ╔══════════════════════════════════════════════════════════════╗"
    echo "  ║  KEVIN'S SWEET FEDORA SETUP COMPLETE                        ║"
    echo "  ╚══════════════════════════════════════════════════════════════╝"
    echo -e "${NC}"

    echo -e "${TEAL}${BOLD}  What was installed:${NC}"
    echo -e "${GREEN}  ✓${NC} Fedora + RPM Fusion (free + nonfree) + Flathub"
    echo -e "${GREEN}  ✓${NC} NVIDIA RTX 5060 (akmod-nvidia open module + Wayland/Vulkan)"
    ...
    echo -e "${GREEN}  ✓${NC} Fastfetch (Fedora logo)"
    echo -e "${GREEN}  ✓${NC} Hyprland + Waybar + Wofi + dunst"
    echo -e "${GREEN}  ✓${NC} DBGate + JetBrains Toolbox (via Flatpak)"
    echo ""
    ...
    echo -e "${PEACH}  5.${NC} Run 'flatpak update' periodically to keep Flatpak apps current"
    echo ""
    ...
    echo -e "  dnfs dnfu dnfr dnfss       — dnf (Fedora)"
    echo -e "  pacs pacu pacr pacss       — pacman-style aliases → dnf"
...
case "${1:-}" in
    ...
    --help|-h)
        cat << USAGE
Usage: ./KevinsSweetFedoraSetup.sh [OPTIONS]
...
Fedora with NVIDIA RTX 5060, Catppuccin Mocha, .NET/Node/Python dev stack.
USAGE
        exit 0 ;;
    "") main ;;
