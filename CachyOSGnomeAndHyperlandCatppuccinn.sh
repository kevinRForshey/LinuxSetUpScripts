...
main() {
    ...
    echo -e "${PEACH}  4.${NC} LOG OUT & BACK IN — GNOME Shell extensions + Shell theme load on relogin"
    echo -e "${PEACH}  5.${NC} Then run: ./KevinsCachyOSSetup.sh --validate  (confirms theme is active)"
    echo -e "${PEACH}  6.${NC} CachyOS Hello app → enable linux-cachyos kernel if not already on it"
    echo ""
